    ...
    for data_split_type in data_split_types:
        ...
        for f in focal_length:
            ...
            for dist in distortion:
                ...
                # TEST
                train_error = cal_error(obj_points_train, img_points_train, rvecs_train, tvecs_train, K_train, dist_train) 
                test_error = cal_error(obj_points_test, img_points_test, rvecs_train, tvecs_train, K_train, dist_train)
                result.append({'train': round(train_error, 3), 'test': round(test_error, 3)})
            results.append(result)   
        
    
        df = pd.DataFrame(results, index= focal_length, columns=distortion)
        frame.append(df)
