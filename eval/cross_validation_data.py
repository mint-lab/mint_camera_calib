...
def filter_img_select(video_file, output_path, cell_size, resolution_levels, chessboard_pattern, dis_thresh, blur_thresh, multi_score_thresh):
    # Open a video
    ...
    save_folder = os.path.join(output_path, 'filter')
    ...
    while True:
        ...
        # Show the image
        cv.putText(img, f'Number Selected Image: {ind}', (10, 25), cv.FONT_HERSHEY_DUPLEX, 0.6, (0, 0, 255))
        ...
        if key == ord(' '):             # Space: Pause and show corners
            ...
            if key == ord('\r'):               
                if laplacian_blur_metric(img) > blur_thresh:                       
                    if len(img_select) == 0:
                        ind += 1
                        img_save_path = os.path.join(save_folder, f'img_{ind}.jpg')
...
if __name__ == '__main__':
    ...
    img_save_folder = 'data/cross_validation_new'
    ...
    if select_type == 'random':
        ...
        img_select = filter_img_select(video_file, output_path=img_save_folder, cell_size=cell_width, chessboard_pattern=chessboard_pattern, resolution_levels=resolution_levels, dis_thresh=0.12, blur_thresh=80, multi_score_thresh=1000)

    # img = cv.imread('img/chessboard_2.png')
    # map = resolution_map(img, (10, 7), resolution_level=2)
    # descriptor = multi_resolution_descriptor(img, (10, 7), resolution_level=2)
    # print(multi_resolution_score(descriptor, resolution_level=2))
    # cv.imshow('map', map)
    # cv.waitKey(0) 
    # cv.destroyAllWindows()
    ...
    #     print('i = ', i)
    #     img1 = cv.imread('random/img_' + str(i) + '.jpg')
    #     for j in range(i + 1, 40, 1): 
    #         img2 = cv.imread('random/img_' + str(j) + '.jpg')
    #         x1 = create_descriptor(img1, 30, chessboard_pattern)
    #         x2 = create_descriptor(img2, 30, chessboard_pattern)
    #         a = np.count_nonzero(x1!=x2)
    #         print(a/len(x1)) 
    #         d = hamming(x1, x2) 
    #         print(d)
    #         print('=====================')
